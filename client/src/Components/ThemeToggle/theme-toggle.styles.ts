...
export const ThemeStyle = styled.div.attrs((props) => ({
  ...
  .Toggle {
    ...
    width: 80px;
    // height: 100px;
    border: 3px solid rgba(252, 250, 250, 0.125);
    ...
    .Button {
      ...
      top: 0px;
      left: 2px;
      transition: left ${(props): string => props.animationSpeed} ease,
        box-shadow ${(props): string => props.animationSpeed} ease,
        border ${(props): string => props.animationSpeed} ease,
        top ${(props): string => props.animationSpeed} ease;
      background: ${(props): string => props.moon1}; /* Old browsers */
      background: linear-gradient(
        135deg,
        ${(props): string => props.moon1} 0%,
        ${(props): string => props.moon2} 50%,
        ${(props): string => props.moon1} 51%,
        ${(props): string => props.moon2} 100%
      ...
      &::before {
        ...
        background: radial-gradient(
          ellipse at center,
          ${(props): string => props.sun1} 5%,
          ${(props): string => props.sun2} 50%,
          ${(props): string => props.sun3} 100%
        ); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
