...
const DropdownMenuDiv = styled.div<any>`
  ...
  box-shadow: -2px -4px 100px -20px rgba(50, 50, 93, 0.25),
...
const DropdownMenu: React.FC<IProps> & NamedChildrenSlots = ({
  children,
  menuLabel,
}) => {
  ...
  return (
    <DropdownMenuDiv open={active === menuLabel}>{children}</DropdownMenuDiv>
  );
};
...
const DropdownItem: React.FC<IDropdownItem> = ({
  ...
  if (onHandleMouseDown && typeof index !== "undefined") {
    return (
      <DropdownItemDiv onMouseDown={(): void => onHandleMouseDown(index)}>
        {children}
      </DropdownItemDiv>
    );
  }
